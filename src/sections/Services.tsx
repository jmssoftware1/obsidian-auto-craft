...
export function Services() {
  ...
  return (
    <section id="servicos" className="border-t border-line bg-bg-alt py-24 md:py-36 lg:py-44">
      <div className="shell">
        <Reveal className="grid grid-cols-1 gap-10 md:grid-cols-12 md:items-end">
          <div className="md:col-span-8">
            ...
            <h2 className="section-title mt-10 max-w-[10ch] font-display font-extrabold uppercase">Uma coleção<br /><span className="text-ink-dim">de precisão.</span></h2>
          </div>
            ...
            <li key={service.number} className={cn(service.featured ? "md:col-span-7 md:col-start-1" : index === 0 ? "md:col-span-4" : index === 2 ? "md:col-span-4 md:col-start-9 md:mt-28" : "md:col-span-4", index === 3 && "md:col-start-2")}>
              <Reveal delay={index * 80}>
                <article className="service-item group">
                  <button type="button" onClick={() => setSelected(service)} className="relative block w-full cursor-zoom-in overflow-hidden bg-bg text-left focus-visible:outline-offset-[-4px]" aria-label={`Ver detalhe de ${service.name}`}>
                    <div className="reveal-image relative overflow-hidden">
      ...
      <ImageLightbox service={selected} onClose={() => setSelected(null)} />
    </section>
  );
}
